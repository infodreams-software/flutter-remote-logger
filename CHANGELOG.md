## 0.3.1
* **iOS ID Persistence**: Implemented Keychain storage (via `flutter_secure_storage` ^10.0.0) for iOS Device ID. This ensures the ID persists even if the app is uninstalled and reinstalled, matching the behavior of `android_id` on Android.
